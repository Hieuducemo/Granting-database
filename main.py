...
#     if subject_name not in [s["name"] for s in data_subject]:
#         data_subject.append({"name": subject_name})

# data_subject_eng = []
# subjects_eng = soup_eng.find_all('a', attrs={'target': '_blank'})
# for subject in subjects_eng:
#     subject_name = subject.text.strip()  # Strip whitespace from subject name
#     if subject_name not in [s["name_en"] for s in data_subject_eng]:
#         data_subject_eng.append({"name_en": subject_name})
        
# data={
#     "acprograms":table_data_program,
#     "acsubjects":data_subject
# }
